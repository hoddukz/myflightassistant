# Tag: core
# Path: backend/app/parsers/airlines/skywest.py
...
from app.parsers.base import BaseICSParser
from app.services.airport import get_timezone


class SkyWestICSParser(BaseICSParser):
    """SkyWest (SkedPlus+) ICS 파서"""

    @staticmethod
    def can_parse(file_content: bytes) -> bool:
        """UID가 Pairing_으로 시작하거나, DESCRIPTION에 Total Block: + Report: 패턴 존재 여부로 판별"""
        try:
            text = file_content.decode("utf-8", errors="ignore")
            has_pairing_uid = "UID:Pairing_" in text
            has_skywest_pattern = "Total Block:" in text and "Report:" in text
            return has_pairing_uid or has_skywest_pattern
        except Exception:
            return False
